...
// create new array from strNums with map, parseInt'ing each position of strNums:
const nums = strNums.map( i => parseInt(i) );
console.log(nums);

// Find the sum of the even values:

// create function to check if number is even and return true or false:
const isEven = (num) => { return num % 2 == 0 };
// create function to use in reduce which adds all numbers together:
const addAllTogether = (accumulator, currentValue) => accumulator + currentValue;
// use filter and reduce with the functions defined above on the nums array:
const sumEvens = nums.filter(isEven).reduce(addAllTogether);
console.log(sumEvens);

// Find the index of the first "Austin" value (the value plus its index equals 512)

// define function to use with findIndex which returns true if the value plus its index equals 512:
const fiveOneTwo = (value, index) => { return value + index == 512 };
// use findIndex with the above function on the nums array:
const atxIdx = nums.findIndex(fiveOneTwo);
